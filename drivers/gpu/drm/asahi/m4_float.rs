...
// Adapted from the Asahi Linux software-float implementation.
...
impl F32U {
    ...
    const fn pack(mut self) -> F32 {
        self = self.norm();
        if !self.is_zero() {
            // Round halfway cases to an even low bit, as the shim
            // does at each binary32 operation.
            self.frac += 0xff + ((self.frac >> 9) & 1);
            self = self.norm();
        }
